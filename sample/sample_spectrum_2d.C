...
void sample_spectrum_2d()
{
	...
	TH2I *h2N_idx_ADCstripU;
	TH2I *h2N_idx_ADCstripD;
	h2N_idx_ADCohmic = new TH2I(
				Form("h2N_idx_ADCohmic"),
				Form("h2N_idx_ADCohmic; Ohmic idx; ADC; count"),
				Nohmic*Ndet, 0, Nohmic*Ndet,	
				0x10000>>3, 0,0x10000);
	h2N_idx_ADCstripU = new TH2I(
				Form("h2N_idx_ADCstripU"),
				Form("h2N_idx_ADCstripU; strip idx; ADC; count"),
				Nstrip*Ndet/2, 0, Nstrip*Ndet/2,	
				0x10000>>3, 0,0x10000);
	h2N_idx_ADCstripD = new TH2I(
				Form("h2N_idx_ADCstripD"),
				Form("h2N_idx_ADCstripD; strip idx; ADC; count"),
				Nstrip*Ndet/2, 0, Nstrip*Ndet/2,	
				0x10000>>3, 0,0x10000);
	...
	for (ient=0; ient<entries; ient++)
	{
		...
		for (x6hit=starkjr_event->v_x6hit.begin(); x6hit!=starkjr_event->v_x6hit.end(); ++x6hit)
		{
			...
			{
				istrip = striphit->idx;
				h2N_idx_ADCstripU->Fill(istrip+ix6*Nstrip/2, striphit->peak);
			}
			for (striphit=x6hit->v_strpDhit.begin(); striphit!=x6hit->v_strpDhit.end(); ++striphit)
			{
				istrip = striphit->idx;
				h2N_idx_ADCstripD->Fill(istrip+ix6*Nstrip/2, striphit->peak);
			}
		}
	}
	...
	pad = c2->cd();
	h2N_idx_ADCstripU->Draw("colz");

	TCanvas *c3 = new TCanvas("c3_StripD","c3_StripD",1200,800);
	pad = c3->cd();
	h2N_idx_ADCstripD->Draw("colz");
