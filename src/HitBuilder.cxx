...
void HitBuilder::ReadMapFile(const char *filename)
{
	...
	{
		map_type		[isid][imid][icha] = 0xFF;
		map_det			[isid][imid][icha] = 0xFF;
		for (itype=0; itype<Ntype; itype++)
			map_idx[itype]	[isid][imid][icha] = 0xFF;
	}
	...
	while (fgets(line, sizeof line, fr))
	{
		...
		else
		{
			...
			for (icha=chL; icha<=chU; icha++)
			{
				...
				map_idx[itype]	[isid][imid][icha] = idx0++;
			}
		}
	}
	for (isid=0; isid<Nsid; isid++)	for (imid=0; imid<Nmid; imid++)
	{
		for(icha=0; icha<Ncha; icha++)
			fprintf(stdout,"%u\t",map_det       [isid][imid][icha]);
		fprintf(stdout,"\n");
	}
}
...
void HitBuilder::SortByDet(vector<Sig> v_sig_coin)
{
	...
	{
		isid = (*it_sig).sid; imid = (*it_sig).mid; icha = (*it_sig).cha;
		itype = map_type		[isid][imid][icha];
		idet  = map_det			[isid][imid][icha];
		iidx  = map_idx[itype]	[isid][imid][icha];

		if(!isvalid(itype, idet, iidx))
		{
			fprintf(stderr,"HitBuilder::SortByDet(vector<Sig> v_sig_coin): not valid\n");
			fprintf(stderr,"HitBuilder::SortByDet(vector<Sig> v_sig_coin): sid %u mid %u cha %u itype %u idet %u iidx %u\n",
					isid, imid, icha, itype, idet, iidx);
			return;
		}
